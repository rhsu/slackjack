...
class RouletteCommandService:
    ...
    def is_valid(self):
        ...
        self.user_data.roulette_bet.append((self.bet_on, self.bet_amount))
        return True, "success"
    ...
    def __valid_color_bet(self):
        ...
        if bet_token in valid_color_bets:
            if bet_token in icon_color_mapping:
                self.bet_on = icon_color_mapping["bet_token"]
            else:
                self.bet_on = bet_token
            return True, None
        else:
    ...
    def __valid_number_bet(self):
        try:
            self.bet_on = int(self.tokens[3])
        except ValueError:
            return False, "Invalid Bet Number"

        if self.bet_on <= 0 or self.bet_on > 28:
            return False, "Invalid Bet Number"
